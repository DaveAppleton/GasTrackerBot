...
func saveUserToDatabase(fName string, lastName string, fullName string, ID int) (exists bool, err error) {
	...
	if err != nil {
		log.Println("check user exists", err)
		return false, err
	}
	if ex {
		log.Println("User", ID, "exists")
		return true, nil
	}
	version := viper.GetFloat64("Version")
	stmtStr := `insert into users (firstname,lastname,fullname,user_id,dateadded,show_level,show_info,version,last_pinged,level_wait)
  values
  ($1,$2,$3,$4,now()::timestamp AT TIME ZONE 'Singapore',false,false,$5,now(),10)
  returning id`
	var id int
...
func saveAlertToDatabase(userID int, alert int) (err error) {
	//query := "insert into gas_levels (user_id,gas_level) values ($1,$2) on conflict on constraint gas_levels_user_id_key do update set gas_level=$2 where gas_levels.user_id=$1 returning id"
	query := "update users set level_wait = $2 where user_id=$1"
	db, err := initDB()
	if err != nil {
		return
	}
	...
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	_, err = db.ExecContext(ctx, query, userID, alert)
	return
}
...
func getAllBelowOrEqual(currentLevel decimal.Decimal) (users []int64, err error) {
	query := "select user_id from users where gas_level >= $1 and show_level and now()-last_pinged > level_wait * interval '1 minutes'"
	db, err := initDB()
	if err != nil {
		return
	}
...
func updateLastPinged(user int64) {
	query := "update users set last_pinged = now() where user_id = $1"
	db, err := initDB()
	if err != nil {
		return
	}
	...
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	if _, err = db.ExecContext(ctx, query, user); err != nil {
		log.Println("could not update last_pinged for user", user)
	}
}
...
func loadWeeklyData() (gds []GasData, err error) {
	query := "select fastest, fast, safelow,average,fastestwait, fastwait, safelowwait,averagewait,blocknum,blocktime, dateadded from gasrates where dateadded > current_date - interval '7' day order by blocknum"
	return loadLatestData(query)
}

func loadDailyData() (gds []GasData, err error) {
	query := "select fastest, fast, safelow,average,fastestwait, fastwait, safelowwait,averagewait,blocknum,blocktime, dateadded from gasrates where dateadded > current_date - interval '1' day order by blocknum"
	return loadLatestData(query)
}

func loadLatestData(query string) (gds []GasData, err error) {
	db, err := initDB()
	if err != nil {
		return
	}
